  ...
  listSanPham.forEach(function (e) {
    if (e.dmsp == "giày nam") {
      ...
      dtnb.innerHTML +=
        ...
        '<a class="card-text text-danger">' + '<p class="text-center">' + (e.price) + 'đ<strike class="text-muted ml-2">'
        + (e.priceold) + '</strike></p>' + '</a>' +
        '<p class="card-text text-center"><button id="cart" data-name="' + e.name + '" data-toggle="modal" data-target="#tthang" class="btn btn-outline-danger"  >Thêm vào giỏ hàng</button></p>' +
        '</div>' +
        '</div>' +
        '</div>';
    }
